...
/* De versiestempel STAMP wordt niet hier gezet maar verderop berekend, uit de
   INHOUD van de bestanden die met ?v= worden aangeroepen. Zie 'versiestempel'. */
...
/* Schrijft alleen als de inhoud echt anders is. Een writeFile met dezelfde inhoud
   geeft het bestand toch een nieuwe wijzigingsdatum, en dan stuurt WinSCP het
   opnieuw op - precies wat we hier proberen te voorkomen. */
async function writeIfChanged(name, body) {
  try { if (await readFile(name, 'utf8') === body) return false; } catch {}
...
await writeIfChanged('app.min.js', banner('app.min.js') + appMin);
...
/* STAMP komt uit de INHOUD van alles wat met ?v= wordt aangeroepen, niet uit de
 * klok. Dat maakt de bouw herhaalbaar: verandert er niets aan de bron, dan komt
 * dezelfde stempel eruit, blijven alle pagina's byte voor byte gelijk, en heeft
 * de upload niets te doen.
 *
 * Hier stond `String(Date.now())`. Daardoor kreeg elke bouw een nieuwe stempel en
 * gingen alle 95 pagina's opnieuw naar de server, ook als je niets had gewijzigd.
 * Dat kostte niet alleen tijd: de voorbeeldstap in upload.bat somde altijd alles
 * op en kon dus nergens meer voor waarschuwen.
 *
 * LET OP: de stempel moet uit CIJFERS bestaan. De pagina's worden opnieuw
...
for (const name of VERSIONED) {
  ...
  digest.update(name + '\0' + body + '\0');
}
const STAMP = BigInt('0x' + digest.digest('hex').slice(0, 12)).toString();
...
/* Vangnet: elk bestand dat met ?v= wordt aangeroepen hoort mee te tellen in de
   stempel. Zo niet, dan verandert de stempel niet als dat bestand wijzigt, en
   houden bezoekers een oude kopie bij nieuwe HTML. Dat is precies de stille fout
   die bovenin dit bestand beschreven staat. */
const dekt = new Set([...VERSIONED, 'app.js', 'styles.css']);
for (const name of [...versionedSeen].sort()) {
  if (!dekt.has(name)) console.log(`LET OP: ${name} wordt met ?v= aangeroepen maar telt niet mee in de stempel - zet hem in VERSIONED`);
}
